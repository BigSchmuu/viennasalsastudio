-- PROJ-44: Ist der eingegebene Code kein Gutschein, wird er als
-- Empfehlungscode geprueft und der Kunde dem Werbenden zugeordnet.
--
-- Die Zuordnung entsteht schon bei der Anfrage, nicht erst beim Bestaetigen.
-- Sie kostet nichts: Das Guthaben haengt an der ersten erfolgreichen
-- Abbuchung, und die gibt es ohne bestaetigte Buchung nie.
create or replace function public.create_regular_course_booking(p_course_id uuid, p_desired_plan text, p_chosen_date date, p_note text, p_prerequisite_confirmed boolean DEFAULT false, p_dance_role text DEFAULT NULL::text, p_coupon_code text DEFAULT NULL::text, p_wants_student_price boolean DEFAULT false, p_terms_accepted boolean DEFAULT false, p_terms_version text DEFAULT NULL::text)
...
begin
  ...
  if p_coupon_code is not null and trim(p_coupon_code) <> '' then
    ...
    -- Dieselben Bedingungen wie in check_coupon_code -- der Hinweis im
    -- Buchungsdialog und das, was hier wirklich passiert, muessen dasselbe
    -- sagen.
    if v_coupon_id is null then
      ...
      then
        -- Nur wenn noch keine Zuordnung besteht. Wer einmal geworben wurde,
        -- wird nicht ein zweites Mal zugeordnet.
        update profiles

-- PROJ-41: Die Anfrage haelt fest, welcher Preis dem Kunden gezeigt wurde.
--
-- Zwei Gruende, ihn hier zu speichern statt beim Bestaetigen neu zu rechnen:
-- der Client koennte einen anderen Betrag unterschieben, und aendert der
-- Betreiber den Standardpreis zwischen Anfrage und Bestaetigung, soll trotzdem
-- der Betrag vorgeschlagen werden, den der Kunde gesehen hat.
--
-- Die alte Signatur wird ausdruecklich entfernt: CREATE OR REPLACE mit einem
-- zusaetzlichen Parameter legt eine Ueberladung an statt zu ersetzen (Lehre
-- aus PROJ-15), und PostgREST wuesste dann nicht, welche gemeint ist.
drop function if exists public.create_regular_course_booking(uuid, text, date, text, boolean, text, text);

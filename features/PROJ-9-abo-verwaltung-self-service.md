...
- **Status:** BEHOBEN (2026-08-17, separate Session nach PROJ-9-Deploy). Root Cause gefunden: Supabase Auth (GoTrue) wirft einen 500er, wenn `auth.users`-Spalten wie `confirmation_token` `NULL` statt Leerstring sind — das DB-Schema erlaubt `NULL`, GoTrues Go-Code aber nicht (verifiziert per `query_logs`: `"error finding user: sql: Scan error on column index 3, name \"confirmation_token\": converting NULL to string is unsupported"`). Alle 11 fehlenden Konten plus fehlende Domain-Fixtures (Kurse/Standorte/Tanzstile/Lehrer/Termine für PROJ-5/PROJ-6) wurden rekonstruiert; zusätzlich wurden 3 durch spätere Features (PROJ-7/8/24) obsolet gewordene Tests neu geschrieben (PROJ-3 und PROJ-23 „Leerer Zustand" — permanent unerfüllbar, da die Produktions-DB jetzt echte Daten enthält; PROJ-5 „Jetzt buchen" — testete einen von PROJ-8 ersetzten Platzhalter) und mehrere reine Locator-Kollisionen mit später hinzugekommener UI behoben (PROJ-2: „Speichern"/„Logout" jetzt mehrfach vorhanden; PROJ-6: „Hinzufügen" jetzt mehrfach vorhanden). **Dabei zusätzlich ein echter Produktionsbug gefunden und behoben:** `SubscriptionManager` (aus PROJ-9 selbst) hatte lokalen State (`useState(initialSubscriptions)`), der nach Anlegen/Löschen eines Abos nie mit den frisch vom Server revalidierten Props synchronisiert wurde — Admins hätten neue/gelöschte Abos ohne manuellen Reload nicht gesehen. Fix: `useEffect`, der lokalen State bei Prop-Änderung synchronisiert. Alle 6 Suiten (45 Tests) laufen jetzt einzeln vollständig grün.
